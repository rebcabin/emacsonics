...
# #+LATEX_HEADER: \lhead{}
# #+LATEX_HEADER: \chead{\textbf{Preliminary Review Draft; Not for Circulation}; feedback to \texttt{bbeckman@gsitechnology.com}}
# #+LATEX_HEADER: \rhead{}
# #+LATEX_HEADER: \lfoot{Confidential}
...
#+TITLE: How to Memorize Random Data -- Revision 2
#+AUTHOR: Brian Beckman
#+DATE: 9 May 2022

* COMMENT PRELIMINARIES

This section won't export to PDF because the string =COMMENT= appears
before the name =PRELIMINARIES=. However, please evaluate the code block
below (=Ctrl-c Ctrl-c= with the cursor inside the block) so you don't
get queried about safety on all the other code block in this document.
...
There is some old stuff and some new stuff in here. Old stuff I
have been using a long time. New stuff I'm experimenting with.

** Pegs and Hallucinations

Most of my methods make mental cartoon movies or pseudo-poems ---
hallucinations -- from picture words. Others (more talented than
I) can make up rhyming or musical lyrics on-the-fly!

My picture words are phonetically on or near my basic, immutable,
two-digit peg list (Table 1). Thanks to my Dad, to Bruno and Lotte
Furst, and to Allan Krill for that list and all others in this
document.

I blogged out the basics of the peg method here:

https://rebcabin.wordpress.com/2010/11/21/memory-pegs/

** Sound-Alikes

Finding a word phonetically near another one is "computing" or
"calculating" a picture word. For example, the peg for 37 is
"mike," and I picture a microphone in my hand and feel the
electrical tingling due to poor grounding in my hand (we've all
experienced that on stage, haven't we?). Adding some synaesthesia
or emotion to pegs helps my memory. Others may not find that
helpful. Do what is best for you!

To calculate some nearby picture words, I get

- mica :: Picture the glittering stone; feel grit on your fingers
  as it crumbles.

- mug :: Feel and taste a big cold beer.
...
To memorize a string of numbers, say a credit-card number, start
with the peg words, which you should recall instantly. For
example, consider the following, fake VISA number from
...
\newpage
There isn't an obvious narrative, here, but we quickly get better
by starting with "fish" (picturesque and smelly) and working
backwards:

- rack DASH :: Take a fishing rod off the RACK and DASH
- lake lane :: down to the LAKE via our favorite LANE.
- fish LEAP :: The FISH are LEAPing today, so we
- MOVE rail :: MOVE the REEL slowly (on the fishing rod).

Krill calls such hallucinations, "phantasmagorishes," a word that,
itself, encodes 5210 3746, a near pangram of the system, missing
only the digits 1 and 9.

** Building and Revising Chains

The most time-consuming aspect of building hallucinations is to
chain the associations so that ideas flow across the movie or
pseudo-poem easily. It's bad to have an idea-chain that breaks
part-way-through, causing a pause or even failure in recall. A
break or even a soft link in a chain is cause for starting over
and building a new chain.

** One Peg Set, Forever

It's critical that the peg list be permanent. Pick some pegs that
work for you, then NEVER CHANGE THEM. Their recall must be
instantaneous, so that whenever you see or think of a number, you
recall the peg without effort.

I regret changing a few pegs once for teaching purposes because my
originals got dated. For example, 3 for "May" became "me" because
no one today knows what a May-pole is. Every time I see 3,
however, I picture a May-pole instead of myself in a mirror
because I learned May-pole first (from Furst and from my Dad). The
switch slows me down even though I made it long, long ago.
...
My basic 2-digit peg list appears in Table 1, extended from 99 to
127 for ASCII and with nine extra pegs for court cards (Section
[[card-pack]]). The 1-digit list with rules for calculating
sound-alikes appears in Table 2. Vowels and soft consonants like
h, w, and y are free when calculating new words off the list as
you can see in Section [[all-pegs]].

#+ATTR_LATEX: :caption Table 1: Basic Extended 2-digit Pegs
|-----+----------+-----+--------+-----+-----------+-----+------------|
...
|  47 | rack     |  57 | lake   |  67 | jack      |  77 | cake       |
...
#+ATTR_LATEX:  :caption Table 2: Fundamental 1-digit Pegs\\
|-------+------+----------------------------------|
| digit | peg  | alike sounds                     |
|-------+------+----------------------------------|
|     0 | saw  | s, z                             |
|     1 | tea  | d, th, dh                        |
|     2 | Noah | ng (Krill)                       |
...
|     5 | law  |                                  |
|     6 | jaw  | sh, ch, zh                       |
|     7 | key  | g, ng (Furst), (nk is always 27) |
|     8 | fee  | v                                |
|     9 | pea  | b                                |
|-------+------+----------------------------------|
...
One way to encode hex is to map A-F to 010-015:

|-----+-----------+----------+----------+-----------+-----------+----------+
| 010 | sawdust   | seduce   | sideways | stays     | South-Sea | assets   |
|   A | assiduous | citizen  | cities   | hostess   |           |          |
|-----+-----------+----------+----------+-----------+-----------+----------+
| 011 | state     | statue   | steady   | sedate    | steed     | stout    |
|   B | studio    | sit-down | astute   | estate    | hesitate  | iced-tea |
|     | wasted    |          |          |           |           |          |
|-----+-----------+----------+----------+-----------+-----------+----------+
| 012 | sadden    | Satan    | satin    | sedan     | sodden    | sudden   |
|   C | stand     | stain    | standard | stench    | stint     | stein    |
|     | stone     | stun     | stunt    | sweeten   | sit-in    | Sweden   |
|     | Estonia   | astound  | hesitant | pseudonym |           |          |
|-----+-----------+----------+----------+-----------+-----------+----------+
| 013 | sediment  | sodium   | stamp    | steam     | stem      | stomach  |
|   D | stampede  | stamina  | stump    | esteem    | estimate  | wisdom   |
|-----+-----------+----------+----------+-----------+-----------+----------+
| 014 | satire    | Saturn   | Saturday | setter    | stair     | star     |
...
|     | history   | hysteria | oyster   | ouster    |           |          |
|-----+-----------+----------+----------+-----------+-----------+----------+
...
A picturesque pseudo-poem about wildlife for this one is easiest.

#+begin_verse
OWL.5 of WISDOM.D and SASSY.00 STARING.E eyes,
HAWK.7, look SIDEWAYS.A left and SIDEWAYS.A right.
STAND.C TALL.15 SUDDEN.C HARRIER.44
and ASSIDUOUS.A OWL.5!

PUMA.93, STEALthy.F CITIZEN.A, SEDUCE.A the HARE.4,
STAMP.D! STRIKE.E! SHAKE.67, LASSIE.50!
Don't HESITATE.B. ENOUGH.28, CITIZEN.!
#+end_verse

Use a little repetition to memorize it. Do it a few times
and check yourself tomorrow. I easily remembered it overnight.
...
digits that spell in 10-15, use two 1-digit pegs, as from Table 2.
For example, don't spell 15 as TALL; that's for F exclusively.
Spell it as TOE EEL or some such.
...
#+begin_example
Law Tam Sauce Tar Key Toes Toes
Tan Tea Tail Tan
Rear Toes Law
Beam Tail Toes
Toes Ray Tam Tar Jack Lace Tot Reef Toes
#+end_example

This will need some work. Ignore 'ing' in the following, wherein
we are assaulted by a traffic cop:

#+begin_quote
A Scottish policeman (LAW [5]) wearing a TAM [D] o'Shanter gives
me SASS [00]. Pulls me over on the TARmac [E] and demands my KEY
[7]. I TOSS [A] it on his TOES [A].

"DONE [C] IT [1] WELL [5], laddie!" THEN [C] ROARING [44], "'TIS
[A] LOW [5], you BUM [93]! 'TILL [F] he EATS [A] TAZER [A4]!'"

"DAMN [D] TAR [C]! SHAKE [67] LESS [50]! TIGHT [B] and ROUGH [28]
on my TOES [A]!"
#+end_quote

I prefer method 1. It produces more compact hallucinations and It
extends to higher radices, as we see in Section [[public-keys]].

\clearpage
* SHA hexes
<<sha-hexes>>

For git, memorizing thefirst 8 hexits are almost always enough,
but let's do the whole thing for an athletic exercise, using the
tables in Section [[all-pegs]].

From https://onlinehashtools.com, consider the following SHA-1
hash:
...
#+begin_verse
SIDEWAYS.A CHAMFER.638?
the TOE.1 of this STATUE.B is a SHAM.63
SODIUM.D is not ALUMINUM.532!
its SHAPE.69 in the STUDIO.B is SIDEWAYS.A
ARTFUL.418 HISTORY.E

LAY.5 a STOOL.F by the STATUE'S.B TOE.1
the STATUE.B is STAMPED.D "ALICE".50
MAYBE.39 ALICE.5 VOUCHES.860 for the STAMP.D
the STAMP.D of a JACOBIAN.679?
#+end_verse

Not bad for 15 minutes' time, using the tables as a crutch. With
practice, we'll need the tables less and less. It will take more
time to memorize it, but not much. Memorize and check yourself
tomorrow.
...
It can be very useful to memorize a public key. Here is an example:
...
This key is in radix 64 (R64). A method-1 ([[method-1]]) encoding of
"Rixits," reserving 010 through 064, follows:
...
15-th capital glyph of the alphabet (see Table 3). Adding 10 for
the first ten digits of the encoding, $15+A = 15+10 = 25$",
not 24. This is an opportunity for error at speed. But we can deal
with it.
...
If you've memorized ASCII (see Table 3 and Section [[method-4]]), you
already have pegs for A-Z and a-z. They will pop up at speed. You
can either force them away; use them as with Method 2 above,
accepting the trade-off risk of collisions and ambiguity; or go
with my Method 4 ([[method-4]]).

** Structure in Public Keys
<<key-structure>>
Here is the original public key from above:
...
Generating another public key on the same machine, we notice that
the first 6 quads, 24 Rixits, 64 bits each, 1536 bits, are the
same in the two keys:
...
By induction, we assume without confirmation or authority that we
need to remember 24 Rixits for the machine and $11\times{}4=44$
Rixits for the rest. That's 2816 bits.
...
<<machine-prefix>>
We don't need to bother with the leading and trailing =AAAA=,
assuming they're always present (TODO: check that!),
leaving us with =C3Nz aC1l ZDI1 NTE5=:

#+begin_verse
SATIN.C HEM.3 in a CINEMAtic.N SASH-WEIGHT.z
SMOOCH.a - STAINED.C, a HOT.1 OATH.1
a SMALL.Z STEAMing.D from the STOVE.I, with TEA.1 brewing
CYANAMIDE.N for a SNOBbish.T SUITOR's.E ALE.5
#+end_verse
...
#+begin_verse
STUFF.I the STILL.F for his ZERO-HOUR.i HURRAH.4
SWEETEN.C the SALT.p with a SCENT.L of SNUFF.S
a SERUM.h SINFUL.S and SINISTER.K for his SASHIMI./
a SOLUTION.u SEMISWEET.U, no SLUDGE.u in this SLOP.x

SATED.B, he SLIPS.x into ASSONANT.M WHIMsies.3
HUSHed.6 SOMNOLENCE.W, SOULFUL.w SNOOZing.K
SATIN.C HUSH.6, SNUG.R STOMACH.D
DIE.1, SWITCHMAN./ ! SNATCH.Q CERBERUS.n

SLOWLY.t SEMIMOBILE.X, SNITCH.Q ! SURRENDER.g !
SEDATED.B, to the CEMETERY.V HOME.3, SERF.m !
SLIDE.p into OSMOSIS.U, ASSIMILATE.Z. SLAIN.q !
#+end_verse
...
Works best for me personally because I already have automatic
NATO, ASCII, and Hebrew mappings for the alphabet from my daily
practice regimen ([[daily-practice]]). Requires no mental arithmetic,
for /me/. Add the pegs for 0..9, plus 62 for "+" and 63 for "/".

|---+----------+-------+---------------------------------------------------|
|   | NATO     | ASCII | hallucination; ASCII pair refs marked with *      |
|   |          |       | Hebrew pegs capitalized                           |
|---+----------+-------+---------------------------------------------------|
| A | ALPHA    | jail  | the alpha-male lion (Aryeh:1) is in jail          |
| B | BAKER    | judge | passing judgment on the lion                      |
| C | CHARLIE  | chalk | hopscotch on the sidewalk                         |
| D | DELTA    | chef  | PhD-thesis* student's day job                     |
| E | ECHO     | chip  | a poker chip in a policeman's(5) hand             |
| F | FOXTROT  | case  | full of a dozen* pink (Vered:6) roses             |
| G | GOLF     | cat   | on the golf cart with my twosome*                 |
| H | HOTEL    | can   | ashtray outside in the smoking area               |
| I | INDIA    | cam   | diesel* Rolls-Royce in a junkyard in India        |
| J | JULIET   | car   | Juliet walks a wood-sage* to his NASCAR ride      |
| K | KILO     | coal  | Alice(11) gives coal to a tusked* troll           |
| L | LIMA     | cage  | containing tanned(12) beach-goers                 |
|---+----------+-------+---------------------------------------------------|
| M | MIKE     | cake  | an orca (Mayim:13) flips a cake on its nose       |
| N | NOVEMBER | cuff  | a snake (Nakhash:14) no arms, cuffs on shirt      |
| O | OSCAR    | cap   | a teetotaler wearing a Carrie-Nation cap          |
| P | PAPA     | face  | Dad cries (tissue:16) at Liz Taylor's face        |
| Q | QUEBEC   | fat   | did Pierre Trudeau get fat?                       |
| R | ROMEO    | fan   | Romeo hands a fan to a Tatar* beauty              |
| S | SIERRA   | fame  | on stage at my book-signing, title* Sierra        |
| T | TANGO    | fare  | a death-watch* soldier gets on the bus            |
| U | UNIFORM  | fall  | a cadet(club jack) falls into a net(21)           |
| V | VICTOR   | fish  | the Christian symbol, nun(22), auto-da-fe*        |
| W | WHISKEY  | fig   | a Few Figs from Thistles; reading it with a dram  |
| X | X-RAY    | fife  | Nero(24) plays the fife, dances* while Rome burns |
| Y | YANKEE   | fob   | a watch-fob hanging from a nail(25) on a US map   |
| Z | ZULU     | base  | an Athenian* ball-player at bat on home base      |
|---+----------+-------+---------------------------------------------------|
...
|   |          |             | Hebrew pegs capitalized                              |
|---+----------+-------------+------------------------------------------------------|
| a | alpha    | back        | the jailer* has her back turned to the lion          |
| b | baker    | beef        | the judge* eats a beef dinner in court               |
| c | charlie  | pipe        | smoking while chalking* the hopscotch grid           |
| d | delta    | thesis      | the chef* hasn't finished the thesis yet             |
| e | echo     | toast       | the cop(5) has toast between his fingers             |
| f | foxtrot  | dozen       | a dozen pink (Vered:6) roses in a case*              |
| g | golf     | twosome     | on the golf cart with the cat*                       |
| h | hotel    | dowser      | done smoking outside (ash-can*), dowsing inside      |
| i | india    | diesel      | holding a camshaft* from diesel Rolls-Royce          |
| j | juliet   | wood-sage   | Juliet escorts a druid to its NASCAR ride            |
| k | kilo     | tusk        | Alice(11) gives coal* to a tusked troll              |
| l | lima     | adhesive    | dangle a fly-paper into the cage*                    |
| m | mike     | teaspoon    | drip icing onto the cake* in the water (Mayim:13)    |
|---+----------+-------------+------------------------------------------------------|
| n | november | tights      | the snake (Nakhash:14) has no legs, wears tights     |
| o | oscar    | teetotaler  | wearing the Carrie Nation cap*                       |
| p | papa     | Titan       | protecting Liz Taylor                                |
| q | quebec   | totem       | a fat* Pierre Trudeau kneels before the totem        |
| r | romeo    | Tatar       | Romeo hands a fan* to a Tatar beauty                 |
| s | sierra   | title       | on stage receiving fame* for my book titled Sierra   |
| t | tango    | death-watch | a soldier gets on the bus paying fare* and dancing   |
| u | uniform  | hotdog      | a cadet falls* into a net(21) holding a hotdog       |
| v | victor   | auto-da-fe  | a Christian* nun(22) overlooks the ritual            |
| w | whiskey  | teddy bear  | while reading a Few Figs* from Thistles              |
| x | x-ray    | dance       | Nero(24) plays fife*, looks through dancers' clothes |
| 7 | yankee   | donut       | hangs on the nail(25) next to the fob*               |
| z | zulu     | Athenian    | on home base* at bat                                 |
|---+----------+-------------+------------------------------------------------------|

Here is the second key from Section [[key-structure]] above:
...
Skipping the AAAA bits, tell a tale of dressing in the mirror,
then playing baseball at lunch time.

*MACHINE STANZA*

=C3Nz= a CHALK sketch of ME, putting on CUFF links and looking
down my ATHENIAN nose.

=aC11= My BACK to the mirror now, another CHALK sketch? My TIE is
TOO tight. But that's enough time dressing in the mirror.

=ZDI1= I'm on BASE now, a CHEF cooks in the bullpen, flipping
burgers with a CAMshaft and brewing TEA.

=NTE5= I COUGH; I lack lunch FARE. Perhaps he'll take poker CHIPs?
But that's against the LAW.

*FIRST STANZA*

=IKWf= The CAMshaft beats the burgers into COAL. FIGs and
a DOZEN roses comes with lunch. To the ballgame!

=n2B6= My baseball TIGHTS ANNOY; says a JUDGE while I spit my
CHAW.

=AjfK= "CHILL like a WOOD-SAGE," I shout. "Consider the ROSES
[dozen], they're COOL!"

=MNwX= The chef adds CAKE and COFFEE, puts everything on a
TEA-TABLE and marks it HALF-OFF. I'm distracted.

*SECOND STANZA*

=+Ekt= The pitcher throws a CHIN-ball, a CHEAP shot. "TISK!" I
shout, "I don't have a DEATH-WISH!"

=d7mx= "DESIST, you HACK!" I'm getting DESPERATE and TENSE.

=nhwi= This is TEDIOUS. DESSERT will help. Out comes the
TEA-TABLE; it's DAZZLING.

=GALP= The KIT is a JEWEL. Do I have the CASH? My FACE turns red.

*LAST STANZA*

=JfEO= JULIET will love the ROSES [dozen], CHEAP to KEEP them?

=oEwk= A TITANic wish! CHEAP still, with a TEDDY-BEAR to the TASK?

=2X/u= NO, a FAVE SHAM DEDICation.

After a few iterations, I can recall this without error. With
practice, it's becoming fun!

Let's try another, the first key from Section [[key-structure]], with
method 4.
...
The machine stanza is the same as before, here abbreviated as
recalled:
...
#+begin_verse
chalk me cufflinks Athenian
back chalk too tight
base chef cam tea
cough fare chips law
#+end_verse
...
#+begin_verse
cam eggs dislike raw
shake tighten catch foam
tosser foam galley jam
hotdog fill hotdog dance
#+end_verse
...
#+begin_verse
judge dance cook ham
jaw fig toothpick call
shock jaw harm chef
doughy gym fat tights
#+end_verse
...
#+begin_verse
hot-dish half-off fat twosome
judge fish me teaspoon
titan fall base teatime
#+end_verse

** Radix 36

Radix 36 works similarly to any method for R64. Tackling R64 gets
you R36 "for free," so we won't cover R36.

* Daily Practice: ASCII,  Cards, NATO, Hebrew, Dates
<<daily-practice>>

Table 3 is my daily repetition-reinforcement regimen. It exploits
the coincidence that there are 52 glyphs in the ASCII Latin
alphabet, 52 cards in a standard pack, and 52 weeks of the year.
There are only 22 Hebrew letters, so the last four rows in each
super-group of 26 are empty in the Hebrew column.

Table 3 is broken into groups of 13 corresponding to the suits of
playing cards. It's fortuitous that each group also contains dates
in a monthly pattern of 5-4-4: 5 weeks in Jan, 4 in Feb, 4 in Mar,
etc. The first days of the months are in the easy pattern 155,
274, 263, 153. You may memorize this as a honeymoon tale of
TAILLIGHTS, NIAGRA, UNASHAMED, HOTELMAN if you like. These are for
non-leap years only, so not great for computing day-of-the-week. I
have another method for that (debugged Furst) in Section
[[day-of-week]].

My daily regimen includes another $156==3\times{}52=2\times{}78$
three columns of hallucinations for the 156 faces of a standard
Tarot pack, obverse and reverse. That's a little arcane for now,
so I don't include it. Maybe in a future Appendix.

I go down Table 3 in my mind when I need to fall asleep. I usually
...
#+ATTR_LATEX:  :caption Table 3: ASCII, Playing cards, NATO, Hebrew, Dates
|-------+---+-----------+-----+-------+--------+----------+---------+--------|
| ASCII |   | CARDS     | NUM | SUIT  | PEG    | NATO     | HEBREW  | DATE   |
|       |   |           |     | + PIP |        |          |         |        |
|-------+---+-----------+-----+-------+--------+----------+---------+--------|
| jail  | A | bat       |   1 | SA    | TEA    | ALPHA    | ARYEH   | JAN 01 |
| judge | B | bean      |   2 | S2    | NOAH   | BAKER    | BAYIT   | JAN 08 |
| chalk | C | beam      |   3 | S3    | ME     | CHARLIE  | GAMAL   | JAN 15 |
| chef  | D | bar       |   4 | S4    | RAY    | DELTA    | DALET   | JAN 22 |
| chip  | E | ball      |   5 | S5    | LAW    | ECHO     | HAR     | JAN 29 |
| case  | F | badge     |   6 | S6    | JAW    | FOXTROT  | VERED   | FEB 05 |
| cat   | G | back      |   7 | S7    | KEY    | GOLF     | ZAHAV   | FEB 12 |
| can   | H | beef      |   8 | S8    | FEE    | HOTEL    | CHAMOR  | FEB 19 |
| cam   | I | pipe      |   9 | S9    | PEA    | INDIA    | TAYAS   | FEB 26 |
| car   | J | base      |  10 | ST    | TOES   | JULIET   | YAD     | MAR 05 |
| coal  | K | potato    |  11 | SJ    | TOT    | KILO     | KISEH   | MAR 12 |
| cage  | L | baton     |  12 | SQ    | TAN    | LIMA     | LECHEM  | MAR 19 |
| cake  | M | podium    |  13 | SK    | TAM    | MIKE     | MAYIM   | MAR 26 |
|-------+---+-----------+-----+-------+--------+----------+---------+--------|
| cuff  | N | rat       |  14 | HA    | TAR    | NOVEMBER | NACHASH | APR 02 |
| cap   | O | rain      |  15 | H2    | TAIL   | OSCAR    | SEFER   | APR 09 |
| face  | P | ram       |  16 | H3    | TISSUE | PAPA     | AYIN    | APR 16 |
| fat   | Q | rear      |  17 | H4    | TACK   | QUEBEC   | PANIM   | APR 23 |
| fan   | R | rail      |  18 | H5    | TAFFY  | ROMEO    | TZIPPOR | APR 30 |
| fame  | S | rash      |  19 | H6    | TAP    | SIERRA   | QUF     | MAY 07 |
| fare  | T | rack      |  20 | H7    | NOSE   | TANGO    | ROSH    | MAY 14 |
| fall  | U | reef      |  21 | H8    | NET    | UNIFORM  | SHAD    | MAY 21 |
| fish  | V | rope      |  22 | H9    | NUN    | VICTOR   | TANIN   | MAY 28 |
| fig   | W | race      |  23 | HT    | NAME   | WHISKEY  |         | JUN 04 |
| fife  | X | rat-a-tat |  24 | HJ    | NERO   | X-RAY    |         | JUN 11 |
| fob   | Y | rattan    |  25 | HQ    | NAIL   | YANKEE   |         | JUN 18 |
| base  | Z | radium    |  26 | HK    | NICHE  | ZULU     |         | JUN 25 |
|-------+---+-----------+-----+-------+--------+----------+---------+--------|
...
|             |   |            |     | + PIP |       |          |         |        |
|-------------+---+------------+-----+-------+-------+----------+---------+--------|
| back        | a | cat        |  27 | CA    | NECK  | alpha    | aryeh   | JUL 02 |
| beef        | b | can        |  28 | C2    | NAVY  | baker    | bayit   | JUL 09 |
| pipe        | c | cam        |  29 | C3    | NAP   | charlie  | gamal   | JUL 16 |
| thesis      | d | car        |  30 | C4    | MASS  | delta    | dalet   | JUL 23 |
| toast       | e | coal       |  31 | C5    | MAT   | echo     | har     | JUL 30 |
| dozen       | f | cage       |  32 | C6    | MAN   | foxtrot  | vered   | AUG 06 |
| twosome     | g | cake       |  33 | C7    | MAMA  | golf     | zahav   | AUG 13 |
| dowser      | h | cuff       |  34 | C8    | MARE  | hotel    | chamor  | AUG 20 |
| diesel      | i | cap        |  35 | C9    | MAIL  | india    | tayas   | AUG 27 |
| wood-sage   | j | case       |  36 | CT    | MATCH | juliet   | yad     | SEP 03 |
| tusk        | k | cadet      |  37 | CJ    | MIKE  | kilo     | kiseh   | SEP 10 |
| adhesive    | l | katana     |  38 | CQ    | MUFF  | lima     | lechem  | SEP 17 |
| teaspoon    | m | catamaran  |  39 | CK    | MAP   | mike     | mayim   | SEP 24 |
|-------------+---+------------+-----+-------+-------+----------+---------+--------|
| tights      | n | tot        |  40 | DA    | RACE  | november | nachash | OCT 01 |
| teetotaler  | o | tan        |  41 | D2    | RAT   | oscar    | sefer   | OCT 08 |
| titan       | p | tam        |  42 | D3    | RAIN  | papa     | ayin    | OCT 15 |
| totem       | q | tar        |  43 | D4    | RAM   | quebec   | panim   | OCT 22 |
| tatar       | r | tail       |  44 | D5    | REAR  | romeo    | tzippor | OCT 29 |
| title       | s | tissue     |  45 | D6    | RAIL  | sierra   | quf     | NOV 05 |
| death-watch | t | tack       |  46 | D7    | RASH  | tango    | rosh    | NOV 12 |
| hot-dog     | u | taffy      |  47 | D8    | RACK  | uniform  | shad    | NOV 19 |
| auto-da-fe  | v | tap        |  48 | D9    | REEF  | victor   | tanin   | NOV 26 |
| teddy-bear  | w | toes       |  49 | DT    | ROPE  | whiskey  |         | DEC 03 |
| dance       | x | teetotaler |  50 | DJ    | LACE  | x-ray    |         | DEC 10 |
| doughnut    | y | titan      |  51 | DQ    | LOT   | yankee   |         | DEC 17 |
| Athenian    | z | totem      |  52 | DK    | LANE  | zulu     |         | DEC 24 |
|-------------+---+------------+-----+-------+-------+----------+---------+--------|
...
a a RAY gun and that ended the peace talks. April 12, 1861.
...
The method is very similar to method 4 above ([[method-4]])
...
applications [method 4 for public keys is almost the same].
...
remember.  If you want peg words, say “ages run in sequence.”  To
learn more about pegs, search for “peg” in my blog.  Because 1900
is 0, 2000 is 6, 2100 will be 4, 2200 will be 2, etc.  1800 is 2.
...
Now get the number of leap years in the century.  That's 72 / 4, or 18,
which is 4.  Add it to the 4 we're keeping, which is 8, which is 1.
...
\clearpage
* APPENDIX: All Pegs and Sound-Alikes
<<all-pegs>>

Letters for hex, ASCII, and Radix-64 method-1 are included in the
row headers.
...
|    | waste   | west   | whist     |          |          |          |
|----+---------+--------+-----------+----------+----------+----------|
| 02 | sane    | scene  | seen      | sign     | sin      | soon     |
|    | son     | snow   | sun       | swan     | swine    | sinew    |
...
|   F | steel     | still     | stool      | style      | sadly     | hostile    |
|-----+-----------+-----------+------------+------------+-----------+------------|
| 016 | stage     | station   | stitch     | stodgy     | stooge    | hesitation |
|   G | hostage   | wastage   |            |            |           |            |
|-----+-----------+-----------+------------+------------+-----------+------------|
| 017 | stack     | stag      | stake      | stalk      | steak     | stick      |
|   H | sticky    | stock     | sting      | stoic      | stoke     | stucco     |
|     | stigma    | setting   | sciatica   | ascetic    |           |            |
|-----+-----------+-----------+------------+------------+-----------+------------|
| 018 | staff     | stave     | stiff      | stove      | stuff     | sea-devil  |
|   I | set-off   | wasteful  |            |            |           |            |
|-----+-----------+-----------+------------+------------+-----------+------------|
| 019 | stab      | steep     | step       | steppe     | staple    | steeple    |
...
\clearpage
|-----+-------------+------------+------------+-----------+-----------+-------------|
| 020 | sense       | science    | since      | sincere   | sinister  | sinus       |
|   K | seance      | sunset     | census     | easiness  | essence   | iciness     |
|     | usance      | snooze     |            |           |           |             |
|-----+-------------+------------+------------+-----------+-----------+-------------|
| 021 | sand        | saint      | sandal     | sanity    | sent      | sentiment   |
|   L | senate      | sonata     | sonnet     | sound     | snoot     | snout       |
|     | sundae      | Sunday     | synod      | scent     | cent      | zenith      |
|     | swindle     | ascent     | assent     |           |           |             |
|-----+-------------+------------+------------+-----------+-----------+-------------|
| 022 | sea-onion   | snow-knife | synonym    | asinine   | singing   | signing     |
|   M | sinning     | assonant   | assonance  |           |           |             |
|-----+-------------+------------+------------+-----------+-----------+-------------|
| 023 | sea-anemone | snowman    | cinema     | cinnamon  | cyanamide | wisenheimer |
|   N |             |            |            |           |           |             |
|-----+-------------+------------+------------+-----------+-----------+-------------|
| 024 | scenery     | senior     | sinner     | snare     | sneer     | snore       |
|   O | snort       | snarl      | sonorous   | sunrise   | sonar     |             |
|-----+-------------+------------+------------+-----------+-----------+-------------|
| 025 | senile      | snell      | snail      | sunlamp   | sunlight  |             |
|   P |             |            |            |           |           |             |
|-----+-------------+------------+------------+-----------+-----------+-------------|
| 026 | sensual     | singe      | snatch     | snitch    | snowshoe  | snow-job    |
|   Q | sunshine    | censure    | cinch      | century   | ascension |             |
|-----+-------------+------------+------------+-----------+-----------+-------------|
| 027 | scenic      | sink       | sea-anchor | snack     | snake     | sneak       |
|   R | snug        | synagogue  | zinc       | isinglass | cynic     |             |
|-----+-------------+------------+------------+-----------+-----------+-------------|
| 028 | sinful      | snaffle    | snafu      | sniff     | snuff     | snowflake   |
|   S | sniffle     | snuffle    |            |           |           |             |
|-----+-------------+------------+------------+-----------+-----------+-------------|
| 029 | sign-up     | snap       | snipe      | snob      | snoop     | snub        |
|   T | snowball    | synopsis   | snip       | sunup     |           |             |
|-----+-------------+------------+------------+-----------+-----------+-------------|
...
|-----+-----------+-------------+------------+------------+-------------+------------|
| 031 | Semite    | smite       | smith      | smooth     | smut        | summit     |
|   V | cemetery  | symmetric   | assumed    | housemaid  |             |            |
|-----+-----------+-------------+------------+------------+-------------+------------|
| 032 | sameness  | seaman      | salmon     | seminary   | semantics   | somnolent  |
|   W | summon    | cement      | iceman     | yes-man    | awesomeness | wisewoman  |
|     | seeming   |             |            |            |             |            |
|-----+-----------+-------------+------------+------------+-------------+------------|
| 033 | semimetal | semimonthly | osmium     | semimat    | semimember  | semimobile |
|   X | semimoist |             |            |            |             |            |
|-----+-----------+-------------+------------+------------+-------------+------------|
| 034 | samurai   | simmer      | smart      | smear      | smirk       | summer     |
|   Y | swimmer   | summary     |            |            |             |            |
|-----+-----------+-------------+------------+------------+-------------+------------|
| 035 | sawmill   | simile      | sea-mile   | seemly     | similar     | small      |
|   Z | smell     | smile       | seamless   | assimilate |             |            |
|-----+-----------+-------------+------------+------------+-------------+------------|
| 036 | smash     | smidgen     | so-much    | smudge     | smooch      |            |
|   a |           |             |            |            |             |            |
|-----+-----------+-------------+------------+------------+-------------+------------|
| 037 | smack     | smoke       | smuggler   | seeming    | swimming    | sumac      |
|   b |           |             |            |            |             |            |
|-----+-----------+-------------+------------+------------+-------------+------------|
| 038 | samovar   | semaphore   | semifinal  | semiformal | symphony    |            |
|   c |           |             |            |            |             |            |
|-----+-----------+-------------+------------+------------+-------------+------------|
| 039 | samba     | sample      | simple     | somber     | somebody    | swamp      |
|   d | symbol    | sympathy    | zombie     | cymbal     | assembly    |            |
|-----+-----------+-------------+------------+------------+-------------+------------|
...
|-----+-------------+--------------+--------------+------------+-----------+-----------|
| 042 | serenade    | serene       | siren        | Syrian     | surname   | surrender |
|   g | sworn       |              |              |            |           |           |
|-----+-------------+--------------+--------------+------------+-----------+-----------|
| 043 | sermon      | serum        | swarm        | ceramic    | ceremony  |           |
|   h |             |              |              |            |           |           |
|-----+-------------+--------------+--------------+------------+-----------+-----------|
| 044 | searer      | sorority     | sour-herring | swearer    | zero-hour |           |
|   i |             |              |              |            |           |           |
|-----+-------------+--------------+--------------+------------+-----------+-----------|
| 045 | serial      | sirloin      | seraglio     | sorrel     | Israel    | cereal    |
|   j |             |              |              |            |           |           |
|-----+-------------+--------------+--------------+------------+-----------+-----------|
| 046 | serge       | sergeant     | search       | surcharge  | surge     | surgeon   |
|   k | sea-urchin  |              |              |            |           |           |
|-----+-------------+--------------+--------------+------------+-----------+-----------|
| 047 | sarong      | sarcasm      | sauerkraut   | sour-cream | sirocco   | circus    |
|   l | circuit     | circumstance | circle       | zircon     |           |           |
|-----+-------------+--------------+--------------+------------+-----------+-----------|
| 048 | serf        | serve        | servile      | surf       | surface   | surfeit   |
|   m | survive     | survey       | servant      | sorrowful  | serafic   |           |
|-----+-------------+--------------+--------------+------------+-----------+-----------|
| 049 | sauerbraten | sourpuss     | surpass      | surplice   | surplus   | serpent   |
|   n | surprise    | syrup        | Cerberus     | usurper    |           |           |
|-----+-------------+--------------+--------------+------------+-----------+-----------|

\clearpage
|-----+------------+-----------+-----------+-----------+-------------+-----------|
| 050 | sal-soda   | sales     | slice     | sluice    | solace      | solicitor |
|   o | solstice   | solus     | solos     | useless   |             |           |
|-----+------------+-----------+-----------+-----------+-------------+-----------|
...
|-----+------------+-----------+-----------+-----------+-------------+-----------|
| 052 | saline     | saloon    | sea-lane  | sealant   | sea-lion    | slain     |
...
|-----+------------+-----------+-----------+-----------+-------------+-----------|
| 054 | salary     | sailor    | seller    | sealer    | slayer      | solar     |
|   s | cellar     | celery    | celerity  | whistler  |             |           |
|-----+------------+-----------+-----------+-----------+-------------+-----------|
| 055 | sea-lily   | slowly    | slyly     | soliloquy | cellular    | celluloid |
|   t |            |           |           |           |             |           |
|-----+------------+-----------+-----------+-----------+-------------+-----------|
| 056 | slash      | slouch    | sludge    | slush     | solution    | zoology   |
|   u | isolation  |           |           |           |             |           |
|-----+------------+-----------+-----------+-----------+-------------+-----------|
| 057 | slack      | slake     | slang     | sling     | sleek       | sea-legs  |
|   v | select     | silk      | silica    | sulky     | ceiling     |           |
|-----+------------+-----------+-----------+-----------+-------------+-----------|
| 058 | salve      | salvo     | saliva    | salvage   | salvation   | slave     |
|   w | sleeve     | self      | solve     | silver    | sliver      | sylph     |
|     | soulfood   | soulful   | sulfur    | sluff     |             |           |
|-----+------------+-----------+-----------+-----------+-------------+-----------|
| 059 | sailboat   | slab      | slap      | sleep     | slip        | slipper   |
|   x | sloop      | slop      | slope     | slowpoke  | celibacy    | celebrate |
|-----+------------+-----------+-----------+-----------+-------------+-----------|

\clearpage
|-----+---------------+-------------+---------------+-------------+-------------+-------------|
| 060 | sash-saw      | sages       | sieges        | sashes      | sea-chest   | suggest     |
|   y | switches      |             |               |             |             |             |
|-----+---------------+-------------+---------------+-------------+-------------+-------------|
| 061 | Sagittarius   | sagittal    | satiate       | sash-weight | siege-tower | soshed      |
|   z | switch-hitter | associate   | sachette      |             |             |             |
|-----+---------------+-------------+---------------+-------------+-------------+-------------|
| 062 | sash-window   | sashoon     | seashine      | sea-change  | sedge-hen   | session     |
|   + | scission      | suasion     | switch-knife  | cession     |             |             |
|-----+---------------+-------------+---------------+-------------+-------------+-------------|
...
|     | Saxon     | squeeze   |           |          |             |           |
|-----+-----------+-----------+-----------+----------+-------------+-----------|
| 071 | scat      | scatter   | skate     | squatter | skid        | skit      |
|     | skeet     | scout     | scuttle   | squid    | sect        | sea-god   |
|     | socket    | Scot      | ascot     | weskit   |             |           |
|-----+-----------+-----------+-----------+----------+-------------+-----------|
...
\clearpage
|---+--------+---------+--------+---------+--------+--------|
| 0 | saw    | sea     | sew    | sow     | so     | zoo    |
|   | ace    | ayes    | eyes   | a-sea   | as     | ass    |
|   | assay  | easy    | essay  | ice     | is     | ooze   |
|   | us     | use     | has    | hawse   | haze   | his    |
|   | hose   | house   | was    | wise    | woozy  | yes    |
|---+--------+---------+--------+---------+--------+--------|
| 1 | tea    | tee     | tie    | toe     | toy    | tow    |
|   | thaw   | they    | thee   | thigh   | though | day    |
|   | die    | do      | due    | dough   | dye    | at     |
|   | ad     | add     | aid    | auto    | eat    | eddy   |
|   | etui   | it      | idea   | hat     | hate   | heat   |
|   | height | hit     | hot    | hoot    | hood   | head   |
|   | heed   | hide    | hut    | oat     | oath   | odd    |
|   | ode    | watt    | what   | wet     | wit    | wait   |
|   | wade   | wad     | weight | white   | wheat  | weed   |
|   | widow  | wide    | with   | yacht   | youth  |        |
|---+--------+---------+--------+---------+--------+--------|
| 2 | Noah   | no      | now    | new     | nigh   | neigh  |
|   | an     | anew    | annoy  | any     | ana    | ani    |
|   | ennui  | gnu     | gnaw   | hen     | honey  | hyena  |
|   | ion    | in      | inn    | knee    | know   | one    |
|   | own    | wan     | when   | win     | wino   | wine   |
|   | wean   |         |        |         |        |        |
|---+--------+---------+--------+---------+--------+--------|
| 3 | May    | may     | me     | mow     | maw    | aim    |
|   | ammo   | ham     | hem    | him     | hum    | home   |
|   | hymn   | whim    | whom   | womb    | Emmy   |        |
|---+--------+---------+--------+---------+--------+--------|
| 4 | ray    | raw     | roe    | rye     | air    | are    |
|   | area   | aria    | array  | arrow   | awry   | aware  |
|   | aura   | ear     | era    | err     | here   | her    |
|   | hair   | hare    | hear   | heir    | hero   | hour   |
|   | harrow | hurrah  | hurry  | ire     | oar    | or     |
|   | ore    | our     | war    | wear    | ware   | where  |
|   | wire   | worry   | weary  | year    | yore   | your   |
|---+--------+---------+--------+---------+--------+--------|
...
\clearpage
|----+-------------+---------+---------+----------+----------+---------|
| 60 | cheese      | chassis | chase   | chaise   | chess    | choice  |
|    | choose      | ashes   | ages    | Jaycee   | Jewess   | joyous  |
|    | juice       | wages   | schuss  |          |          |         |
|----+-------------+---------+---------+----------+----------+---------|
| 61 | chat        | chateau | cheat   | cheetah  | chit     | chute   |
|    | jade        | jet     | jetty   | jot      | jute     | judo    |
...
|    | wishy-washy |         |         |          |          |         |
|----+-------------+---------+---------+----------+----------+---------|
| 67 | check       | chalk   | cheek   | choke    | chug     | chick   |
...
|   t |             |             |                |            |            |             |
|-----+-------------+-------------+----------------+------------+------------+-------------|
| 117 | hotdog      | toothache   | dating (Krill) | dedicate   | deduct     | detect      |
...
|   x | tinsel       | dense       | dunce       | ad-nauseam   | Adonis       | Athens       |
...
|     | hatcheck     | woodchuck    | staging      | stagecoach   |             |               |
|-----+--------------+--------------+--------------+--------------+-------------+---------------|
