...
#+TITLE: How to Memorize Credit Cards, GUIDs, Hashes, Public Keys, and More --- Revision 4
...
which I merged with the "peg system."

My hallucinations come with synaesthesia: smells, heat, texture, colors,
brightness, etc. I use only imagination, no drugs. Many memory
practitioners do not use synaesthesia, but they help me.
...
My methods are based on the table of

- pegs :: permanent, immutable picture words in one-to-one
  correspondence to the numbers

- sound-alikes :: fluid, mutable list of picture words that are similar
  in consonant sounds to the pegs and in many-to-one correspondence to
  the numbers.

See Section [[all-pegs]] of this document, after the Appendices.

For radices beyond decimal, I choose a mapping from alphabetic glyphs to
ASCII because I already have the mapping memorized for instant,
bidirectional recall (Section [[daily-practice]]).
...
letters. For Radix-64, we need both cases.
...
first in each block and in all-caps. My short-list of preferred
sound-alikes are in the first row of each table.

I don't (yet) have a memorized peg for every number, just for the
most useful ones. I pick pegs that don't collide visually with the
most frequent ones. I already have CHAIR for 64 (used all the
time), so SEAT for 01 and THRONE for 142, both rare, are not good
ideas because they might incorrectly recall 64 at speed.
