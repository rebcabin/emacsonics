...
#+LATEX_HEADER: \lhead{}
#+LATEX_HEADER: \chead{\textbf{Do Not Distribute}}
#+LATEX_HEADER: \rhead{}
#+LATEX_HEADER: \lfoot{Do Not Distribute}
...
#+TITLE: How to Memorize Credit Cards, GUIDs, Hashes, Public Keys, and More
#+AUTHOR: Brian Beckman
#+DATE: 12 May 2022

* Introduction

I memorize numbers in any radix (base) by hallucinating poems, stories,
or mental cartoon movies via the Mnemonic Major System:

https://en.wikipedia.org/wiki/Mnemonic_major_system

which I call the "peg system."

My hallucinations come with synaesthesia: smells, heat, texture, colors,
brightness, etc. I use only imagination, no drugs. Many memory athletes
do not use synaesthesia.

Krill calls the inputs for such hallucinations "phantasmagorishables,"
a portmanteau that encodes the digits 8210 3746 95.

https://folk.ntnu.no/krill/pseudonumerology.htm

My methods are based on the table of pegs (permanent, immutable
picture words) and sound-alikes in Section [[all-pegs]] of this
document, after the Appendices.

For radices beyond decimal, I choose a mapping from alphabetic glyphs to
ASCII because I already have the mapping memorized for instant, bidirectional
recall (Section [[daily-practice]]).

In appendices, I show some alternative mappings for people who
don't have ASCII memorized.
\clearpage
* Decimal Credit-Card Numbers

This is the easiest example in this document. For decimal, the pegs are:

|-------+------+---------------------------------------------------+-------------------------|
| digit | peg  | mnemonic                                          | sound-alike rules       |
|-------+------+---------------------------------------------------+-------------------------|
|     0 | saw  | circular saw, round like "0"                      | s, z                    |
...
|     4 | ray  | this one is har to remember; just brute-force it  |                         |
...
Find specific sound-alikes in Section [[all-pegs]].
...
I could recall this the credit-card number days after devising it,
without refreshing my memory. Devising it took less than a minute.
I instantly memorized it while devising it without extra effort
because I know the entire 2-digit peg table by heart.
...
|---+---------+-------+----+----------------------------------------------|
| A | ALPHA   | jail  | 65 | the alpha-male lion (Aryeh:1) is in jail     |
...
| E | ECHO    | chip  | 69 | a poker chip in a policeman's(5) hand        |
| F | FOXTROT | case  | 70 | full of a dozen* pink (Vered:6) roses        |
|---+---------+-------+----+----------------------------------------------|
...
This is only a little harder than decimal.
...
| 5D00E7AA | law chef sauce chip key jail jail |
| C15C     | chalk tail chalk                  |
| 44A5     | rear jail law                     |
| 93FA     | beam case jail                    |
| A4DE675  | jail ray chef chip chuckle        |
| 0B28A    | saw judge Navy jail               |
...
Hallucinate a "hell-chef sauce," a recipe for a naive (28),
gullible, unsuspecting victim. We seeded this hallucination from that
one word, "naive."

| 5D00E7AA | HELL-chef sauce: CHOP, HACK, and CHILL the JELLY. |
| C15C     | SHAKE with TALLOW, CHECK                          |
| 44A5     | the RARE CHILI OIL.                               |
| 93FA     | "BE-MY HAGGIS, JILL!"                             |
| A4DE675  | CHILL for an HOUR, SHAVE, CHOP, CHUCKLE.          |
| 0B28A    | "ASSAY, JUDGE!" NAIVE JILL.                       |
...
| 860d d679 | FETCH the CHIEF, JEHOVAH for JACOB!           |

\clearpage
* Radix 36: Spack

Radix-36 is more difficult to find "in the wild" than is either
hex or Radix-64. I most often see it in URLs for encoding queries
or other such data. It's useful there because HTTP is not
case-sensitive, so Radix-64 is overkill and sucks up alternatives.

For encoding /rixits/ in radix-36 (or biscuits in base-36?), add the
remaining capital letters and their sound-alikes from Section [[all-pegs]].
Radix-36 is case-insensitive, by design, so we only need one set of
letters. In Radix-64, we need both cases.
...
Spack (https://spack.io/) is a superlative dependency solver for
software build systems. It produces directory names in R36 that
won't (statistically) collide with SHA-1 hashes and work fine on
case-insensitive file systems like some of those on Windows.
...
- "Car" for 74 is not ok because it decodes as "J". Must peg it as
  "key ray," then try sound-alikes.

- "Fish" is "V" in R36, not 86. If we had 86, we would have to
  encode it in pegs as "fee jaw," then try sound-alikes.

- 72 can't be "can," because that's "H" in R36. Encode as "key
  Noah" then try sound-alikes.

\clearpage
* Radix 64

For encoding rixits in base-64, add the lower-case letters plus
special cases for "+", 62, and "/", 63.
...
get the required 68. We have 60 to encode
...
| 1/Qn | DIE, SWITCHMAN! SNATCH CERBERUS              |
|      |                                              |
...
table 2 exploits the coincidence that there are 52 glyphs in the
...
amongst each 26 are empty in the Hebrew column.

table 2 is in groups of 13 corresponding to the suits of playing
cards. Each group also contains dates in a monthly pattern of
5-4-4: 5 weeks in Jan, 4 in Feb, 4 in Mar, etc. The first days of
the months are in the easy pattern 155, 274, 263, 153. You may
memorize this as a honeymoon tale of TAILLIGHTS, NIAGRA,
UNASHAMED, HOTELMAN.
...
I go down table 2 in my mind when I need to fall asleep. I usually
...
#+ATTR_LATEX:  :caption table 2: ASCII, Playing cards, NATO, Hebrew, Dates
|-------+---+-----------+-----+-------+--------+----------+---------+--------|
...
included in the row headers. My personal permanent peg words are
first in each block and in all-caps. I don't (yet) have a
memorized peg for every number, just for the useful ones. I pick
pegs that don't collide visually with the most frequent ones. I
already have CHAIR for 64 (used all the time), so SEAT for 01 and
THRONE for 142, both rare, are not good ideas because they might
incorrectly recall 64 at speed.
...
\clearpage
|-----+---------------+------------+---------------+--------------+-------------+-------------|
| 060 | SAGES         | switches   | sieges        | sashes       | sea-chest   | suggest     |
|   y |               |            |               |              |             |             |
|-----+---------------+------------+---------------+--------------+-------------+-------------|
| 061 | SAGITTARIUS   | sagittal   | satiate       | sash-weight  | siege-tower | soshed      |
|   z | switch-hitter | associate  | sachette      |              |             |             |
|-----+---------------+------------+---------------+--------------+-------------+-------------|
| 062 | SWITCH-KNIFE  | sashoon    | seashine      | sea-change   | sedge-hen   | session     |
|   + | scission      | suasion    | sash-window   | cession      |             |             |
|-----+---------------+------------+---------------+--------------+-------------+-------------|
...
|     | switcheroo    |            |               |              |             |             |
|-----+---------------+------------+---------------+--------------+-------------+-------------|
| 065 | seashell      | sage-oil   | satchel       | sash-line    | switch-line | social      |
|-----+---------------+------------+---------------+--------------+-------------+-------------|
| 066 | sage-judge    | wise-judge | sage-cheese   | switch-shift | association |             |
|-----+---------------+------------+---------------+--------------+-------------+-------------|
| 067 | sedge-grass   | sash-cord  | siege-coin    | switchgear   | switch-cane |             |
|-----+---------------+------------+---------------+--------------+-------------+-------------|
